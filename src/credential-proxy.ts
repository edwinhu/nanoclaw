...
function readFullOAuthCredentials(): OAuthCredentials | undefined {
  ...
  // Fallback: macOS keychain (via keychain.ts — mockable in tests)
  const keychainCreds = readKeychainOAuthCredentials();
  if (keychainCreds?.accessToken) return keychainCreds as OAuthCredentials;
 ...
 * and macOS keychain so the next process restart picks up the new token.
 */
function saveOAuthCredentials(updated: OAuthCredentials): void {
  ...
  try {
    ...
    logger.warn({ err }, 'Failed to persist refreshed OAuth token to file');
  }

  // Also update macOS keychain so Claude Code CLI picks up the refresh
  if (process.platform === 'darwin') {
    try {
      let keychainData: Record<string, unknown> = {};
      try {
        const raw = execSync(
          'security find-generic-password -s "Claude Code-credentials" -w 2>/dev/null',
          { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] },
        ).trim();
        keychainData = JSON.parse(raw);
      } catch {
        // no existing entry
      }
      keychainData.claudeAiOauth = {
        ...((keychainData.claudeAiOauth as object) ?? {}),
        ...updated,
      };
...
export function startCredentialProxy(
  ...
  const getOauthToken = async (): Promise<string | undefined> => {
    ...
    // Re-read from disk/keychain if cache TTL elapsed, then refresh if needed.
